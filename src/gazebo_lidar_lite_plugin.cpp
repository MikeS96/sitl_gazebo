...
#include "gazebo_lidar_lite_plugin.h"
...
GZ_REGISTER_SENSOR_PLUGIN(LidarLitePlugin)

/////////////////////////////////////////////////
LidarLitePlugin::LidarLitePlugin()
{
}
...
void LidarLitePlugin::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  // Get then name of the parent sensor <sensors::RaySensorPtr>
  parentSensor_ = std::dynamic_pointer_cast<sensors::RaySensor>(_parent);

  // Check if the Ray sensor is attached !parentSensor_ = 0
  if (!parentSensor_)
    gzthrow("LidarLitePlugin requires a Ray Sensor as its parent");

  // <physics::WorldPtr>, This is a pointer
  world_ = physics::get_world(parentSensor_->WorldName());

  // <event::ConnectionPtr> Callback, creating connection to the update method
  newLaserScansConnection_ = parentSensor_->LaserShape()->ConnectNewLaserScans(
      boost::bind(&LidarLitePlugin::OnNewLaserScans, this));

  // Ensure that the sensor is active
  parentSensor_->SetActive(true);

  // _sdf->HasElement("robotNamespace") Return 1 or 0
  if (_sdf->HasElement("robotNamespace"))
    namespace_ = _sdf->GetElement("robotNamespace")->Get<std::string>();
  else
    gzwarn << "[gazebo_lidar_lite_plugin] Please specify a robotNamespace.\n";
  ...
  if (_sdf->HasElement("min_distance")) {
    ...
    gzwarn << "[gazebo_lidar_lite_plugin] Using default minimum distance: " << kDefaultMinDistance << "\n";
  ...
  if (_sdf->HasElement("max_distance")) {
    ...
    gzwarn << "[gazebo_lidar_lite_plugin] Using default maximum distance: " << kDefaultMaxDistance << "\n";
  ...
  const string scopedName = _parent->ParentName(); //airfly_hexa::lidar_lite
  vector<string> names_splitted;
  boost::split(names_splitted, scopedName, boost::is_any_of("::")); //split based on ::
  ...
  if(_sdf->HasElement("topic")) {
    ...
    // if not set by parameter, get the topi-->c name from the model name
    // lidar_topic_ = parentSensorModelName;
    lidar_topic_ = "lidar_lite";
    gzwarn << "[gazebo_lidar_lite_plugin]: " + names_splitted.front() + "::" + names_splitted.rbegin()[1] +
...
/////////////////////////////////////////////////
void LidarLitePlugin::OnNewLaserScans()
{
